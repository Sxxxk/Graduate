...
namespace PDollarGestureRecognizer
{
    public class GestureIO
    {
		...
		/// <returns></returns>
		public static Gesture ReadGestureFromFile(string fileName) {//读取手势记录文件

			XmlTextReader xmlReader = null;//读取xml文件
			Gesture gesture = null;//手势
			
			try {
				
				xmlReader = new XmlTextReader(File.OpenText(fileName));//系统读取xml的方法
				gesture = ReadGesture(xmlReader);//读取手势文件，返回手势
		...
		private static Gesture ReadGesture(XmlTextReader xmlReader)//从xml文件中读取手势
        {
            List<Point> points = new List<Point>();
            int currentStrokeIndex = -1;//当前手势编号
            string gestureName = "";
            try
            {
        ...
        /// </summary>
        public static void WriteGesture(PDollarGestureRecognizer.Point[] points, string gestureName, string fileName)//写xml文件
        {
